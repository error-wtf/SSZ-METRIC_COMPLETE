...
def test_observable_method_matrix_routing(obs_id, expected_class, expected_method):
    ...
    print(f"  {obs_id}: class={obs['class']}, method={obs['method']}")
...
def test_entire_registry_routing_rules():
    ...
    allowed_methods = {
        "PPN_COMPLETION", "XI_DIRECT", "PPN_ORBIT",
        "SSZ_KINEMATIC_IDENTITY", "XI_STRONG_FIELD_DIAGNOSTIC"
    }
    allowed_classes = {
        "NULL_LIGHT", "TIMELIKE_STATIC", "TIMELIKE_ORBIT",
        "KINEMATIC_INVARIANT", "STRONG_FIELD_DIAGNOSTIC"
    }
    
    observables = list_observables()
    print(f"  Checking {len(observables)} observables...")
