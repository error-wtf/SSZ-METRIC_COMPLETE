...
def test_observable_classification_routing():
    ...
    # 1. Null/Light path
    print("  Testing NULL_LIGHT classifications (lensing, shapiro, vlbi, group_delay)")
    ...
    print("  Testing TIMELIKE_STATIC classifications (redshift, time_dilation, gps_clock)")
    ...
    # 3. Timelike Orbit
    print("  Testing TIMELIKE_ORBIT classifications (perihelion, precession, frame_dragging)")
...
def test_ppn_completion_evaluation():
    ...
    print(f"  Xi input: {xi_only}")
    print(f"  PPN completed (1+gamma=2): {completed}")
    print(f"  Expected: {xi_only * 2.0}")
